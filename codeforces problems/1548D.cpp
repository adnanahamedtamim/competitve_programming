...
ll query(ll L, ll R, vector<vector<ll>> &lookup) {
    ll j = (ll)log2(R - L + 1);
    return gcd(lookup[L][j], lookup[R - (1LL << j) + 1][j]);
}
...
int main(){
  ...
  while(t--){
     ...
     vll su;
     for(ll i=1;i<n;i++){
         su.pb(abs(ara[i]-ara[i-1]));
     }
     ll m=n-1;
    
     vector<vector<ll>> lookup = buildSparseTable(su); 
     ...
     for(ll i=0;i<m;i++){
       ...
       while(l<=r){
          ...
          // ll zcnt=cntt[mid]-(i > 0 ? cntt[i-1] : 0);
          
           if( query(i,mid,lookup)!=1){
              val=mid;
              l=mid+1;
           }   
           else{
