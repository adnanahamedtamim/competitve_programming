...
while(t--){
    ...
    {
        stk.push(ara[i]);
        if(stk.size()>k){  // there must be at least k+1 free elements to take this one
            ll val=stk.top();
            stk.pop();
            st.insert(val);
            ll cnt=0;
            
            while(cnt<k){  // extract k elements which are accountable to element at i
               stk.pop();
               cnt++;
            }
        }
        else if(st.size()>0 && (*(st.begin()))<ara[i]){  // checks for a better candidate , replacing the previously taken one
            ll val=stk.top();
            stk.pop();
            ll val2=*(st.begin());
            st.erase(st.find(val2));
