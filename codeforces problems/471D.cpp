...
const ll BASE1 = 911382323LL;
const ll BASE2 = 972663749LL;
const ld PI = acos(-1.0L);
const ld EPS = 1e-12;

const int MAXN = 2e5 + 5;
const int MAXSIEVE = 5e5 + 5;
...
int main()
{
    ...
    ll inv1=binexp(BASE1,MOD1-2,MOD1);
    ll inv2=binexp(BASE2,MOD2-2,MOD2);

    for(ll i=0;i<w-1;i++){
          ll d=b[i+1]-b[i];

          ll x=(d%MOD1+MOD1)%MOD1;
          h1=((__int128)p1*x+h1)%MOD1;
          p1=((__int128)p1*BASE1)%MOD1;

          x=(d%MOD2+MOD2)%MOD2;
          h2=((__int128)p2*x+h2)%MOD2;
          p2=((__int128)p2*BASE2)%MOD2;
    }
    ...
    for(ll i=0;i<w-1;i++){
          ...
          ll x=(d%MOD1+MOD1)%MOD1;
          win1=((__int128)p1*x+win1)%MOD1;
          p1=((__int128)p1*BASE1)%MOD1;

          x=(d%MOD2+MOD2)%MOD2;
          win2=((__int128)p2*x+win2)%MOD2;
          p2=((__int128)p2*BASE2)%MOD2;  
    }
  ll ans=0;
  if(win1==h1 && win2==h2){
  ...
  ll highest1=binexp(BASE1,w-2,MOD1);
  ll highest2=binexp(BASE2,w-2,MOD2);


  for(ll i=w-1;i<n-1;i++){
       ll newdd=a[i+1]-a[i];
       ll len=w-1;
       ll oldd=a[i-len+1]-a[i-len];

       //pop front
       win1=(win1-oldd%MOD1+MOD1)%MOD1;
       win1=((__int128)win1*inv1)%MOD1;
       win2=(win2-oldd%MOD2+MOD2)%MOD2;
       win2=((__int128)win2*inv2)%MOD2;


       // push_back
       win1=(win1+(__int128)((newdd+MOD1)%MOD1)*highest1)%MOD1;
       win2=(win2+(__int128)((newdd+MOD2)%MOD2)*highest2)%MOD2;
