...
double func(ll x,ll y,ll h,ll c){
    if(x<0 || y<0) return (LLONG_MAX)*1.0;
    
    double avg=(x*h+y*c)*1.0;
    avg/=(x+y);
...
int main(){
  ...
  ll k=(h-t)/(2*t-h-c); // cold count
   
   auto calc = [&](ll k){
        ll pours = 2*k + 1;
        long double temp = ((k+1)*h + k*c) / (long double)pours;
        return abs(temp - t);
    };
