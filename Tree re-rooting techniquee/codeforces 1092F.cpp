...
void dfs(ll i,ll par,vvll& adj,vll& dist,vll& sum,vll& ara){
     sum[i]+=ara[i];
     for(auto it : adj[i]){
          if(it==par) continue;
          dist[it]=dist[i]+1;
          dfs(it,i,adj,dist,sum,ara);
          sum[i]+=sum[it];
     }
}


void dfs2(ll i,ll par,ll cur,vvll& adj,vll& sum,ll& maxi){
  
  for(auto it : adj[i]){
      if(it==par) continue;
      ll subtree=sum[it];
      ll baki=sum[0]-sum[it];
      ll cur2=cur-subtree+baki;
      maxi=max(maxi,cur2);
      dfs2(it,i,cur2,adj,sum,maxi);
  }
...
int main()
{
   ...
   vll dist(n,0),sum(n,0);

   dfs(0,-1,adj,dist,sum,ara);
