...
struct tree{
 ...
 void build(ll node,ll l,ll r,vll& bit){
  lazy[node]=false; 
  if(l==r){
      cnt[node]=bit[l];
      return;  
  ...
  build(2*node+1,l,mid,bit);
  build(2*node+2,mid+1,r,bit);
 ...
 void update(ll node,ll l,ll r,ll ql,ll qr){
   ...
   ll mid=l+(r-l)/2;
   if(ql<=mid) update(2*node+1,l,mid,ql,qr);
   if(qr>mid)  update(2*node+2,mid+1,r,ql,qr);
 ...
 ll query(ll node,ll l,ll r,ll ql,ll qr,bool passed){
   
    if(ql<=l && qr>=r){
       if(passed) return (r-l+1)-cnt[node];
       return cnt[node];
    }
   ll mid=l+(r-l)/2;
   ll sum=0;
   bool cur_passed=passed^lazy[node];
   if(ql<=mid) sum+=query(2*node+1,l,mid,ql,qr,cur_passed);
   if(qr>mid) sum+=query(2*node+2,mid+1,r,ql,qr,cur_passed);
   return sum;
 }
...
int main()
{
    ...
    for(ll i=0;i<=20;i++){
          ...
          bittree[i].build(0,0,n-1,bits);
    }
    // cout << bittree[0].query(0,0,n-1,1,3,false) << endl;
    ll q;
    cin >> q;
    ...
    while(q--){
       ...
       if(tt==1){
           ...
           ll ans=0;
           for(ll i=0;i<=20;i++){
               ll cntt=bittree[i].query(0,0,n-1,l,r,false);
            //    if(q==0) cout << i <<  " " << cntt << endl;
               ans+=(cntt*(1LL<<i));
           }
           cout << ans << endl;
       }
