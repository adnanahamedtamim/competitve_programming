...
// vector<ll> spf(MAXSIEVE+2);

// void build_spf(){
//     for(ll i=0;i<=MAXSIEVE;i++){
//         spf[i]=i;
//     }
...
void gen(ll idx, ll r, ll sum, vll &ara, vll &v){
    if(idx == r){
        v.pb(sum);
        return;
    }

    gen(idx+1, r, sum, ara, v);
    gen(idx+1, r, sum+ara[idx], ara, v);
}
...
int main()
{
    ...
    for(auto it : a){
         ans+=(upper_bound(all(b),m-it)-lower_bound(all(b),m-it));
    }
    cout << ans << endl;
